...
- `"rules"` is an object keyed by `rule_id` — see
  `core/lwt_core/rules/__init__.py`'s `RULES` list for every rule id a
...
finding (see `core/lwt_core/engine.py`); `warn` findings from rules that
...
- `lwt_core.config.load_policy_dict(None)` — or any non-dict input —
...
- A single rule entry that isn't an object, or whose `"mode"` isn't a
  ...
  other rule in the same file (see `lwt_core/config.py::load_policy_dict`).
- `plugins/claude/lwt/bin/lwt_hook.py` turns "file doesn't exist" and
...
- Claude Code plugin: `LWT_POLICY_PATH` env var if set, otherwise the
  vendored `core/policy/default.json` bundled at
  `plugins/claude/lwt/vendor/policy/default.json`.
- Codex plugin: same convention, read by the `lwt-config` /
  `lwt-report` skills rather than a hook (see `docs/install-codex.md`).
