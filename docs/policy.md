...
- `"rules"` is an object keyed by `rule_id` — see
  `core/lwh_core/rules/__init__.py`'s `RULES` list for every rule id a
...
finding (see `core/lwh_core/engine.py`); `warn` findings from rules that
...
- `lwh_core.config.load_policy_dict(None)` — or any non-dict input —
...
- A single rule entry that isn't an object, or whose `"mode"` isn't a
  ...
  other rule in the same file (see `lwh_core/config.py::load_policy_dict`).
- `plugins/claude/lwh/bin/lwh_hook.py` turns "file doesn't exist" and
...
- Claude Code plugin: `LWH_POLICY_PATH` env var if set, otherwise the
  vendored `core/policy/default.json` bundled at
  `plugins/claude/lwh/vendor/policy/default.json`.
- Codex plugin: same convention, read by the `lwh-config` /
  `lwh-report` skills rather than a hook (see `docs/install-codex.md`).
