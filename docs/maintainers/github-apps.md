# GitHub Apps: lwh-claude, lwh-codex
...
`lwh-claude` is Claude Code's identity, `lwh-codex` is Codex's. Separate
...
Apps operate on `LEAPWare-Software/LEAPWare-Helm` only, nothing
...
2. On that page there is a form that POSTs a manifest to GitHub; use the
   contents of `.github/apps/lwh-claude.json` (or `lwh-codex.json`) as the
...
4. Install the App on `LEAPWare-Software/LEAPWare-Helm` only (not
   "all repositories").
...
- Each laptop that needs to act as `lwh-claude` or `lwh-codex` pulls the key
