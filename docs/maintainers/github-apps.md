# GitHub Apps: lwt-claude, lwt-codex
...
`lwt-claude` is Claude Code's identity, `lwt-codex` is Codex's. Separate
...
Apps operate on `LEAPWare-Software/LEAPWare-TokenWise` only, nothing
...
2. On that page there is a form that POSTs a manifest to GitHub; use the
   contents of `.github/apps/lwt-claude.json` (or `lwt-codex.json`) as the
...
4. Install the App on `LEAPWare-Software/LEAPWare-TokenWise` only (not
   "all repositories").
...
- Each laptop that needs to act as `lwt-claude` or `lwt-codex` pulls the key
