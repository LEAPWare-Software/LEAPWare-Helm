...
true. It has no runtime role — the lwh plugin never reads it.
...
A handoff is cheap and mechanical (`scripts/lwh_handoff.py --write`); the
cost of skipping one is a session that starts from guesswork instead of
fact.

## What the generated block contains

Between `<!-- lwh-handoff:begin -->` and `<!-- lwh-handoff:end -->` in
`HANDOFF.md`, `scripts/lwh_handoff.py --write` regenerates:
...
target — `scripts/lwh_handoff.py --check` fails the build over it. A
...
starting `LWH - Alert: <id> DONE ...` so it is grep-able across a long
session.
