...
true. It has no runtime role — the lwt plugin never reads it.
...
A handoff is cheap and mechanical (`scripts/lwt_handoff.py --write`); the
cost of skipping one is a session that starts from guesswork instead of
fact.

## What the generated block contains

Between `<!-- lwt-handoff:begin -->` and `<!-- lwt-handoff:end -->` in
`HANDOFF.md`, `scripts/lwt_handoff.py --write` regenerates:
...
target — `scripts/lwt_handoff.py --check` fails the build over it. A
...
starting `LWT - Alert: <id> DONE ...` so it is grep-able across a long
session.
