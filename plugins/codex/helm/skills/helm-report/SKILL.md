---
name: helm-report
description: Run helm's policy engine over a described action and report what it WOULD decide (Codex side, reporting-only — no enforcing hook ships here, see docs/install-codex.md).
---
...
the same `helm_core.engine.evaluate` used by the Claude Code plugin, via
...
2. Load the active policy the same way helm-config does.
3. Run `adapters.codex.hook_io.parse_event`, then `helm_core.engine.evaluate`,
