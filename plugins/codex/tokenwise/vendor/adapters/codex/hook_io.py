...
That is not a footing TokenWise ships a DENY-capable gate on: a hook a user must
manually trust per-session, in an undocumented manifest shape, cannot be the
mechanical policy layer this project promises.

So the Codex plugin ships two skills (tokenwise-config, tokenwise-report) and this
...
from tokenwise_core.config import Policy, load_policy_dict
from tokenwise_core.engine import Decision
...
def render_report(decision: Decision) -> str:
    """Human-readable text for the tokenwise-report skill. Reporting only — never blocks."""
    lines = []
    if decision.permit:
        lines.append("tokenwise: would ALLOW under current policy.")
    else:
        lines.append(f"tokenwise: would DENY under current policy — {decision.deny_reason}")
    for warning in decision.warnings:
        lines.append(f"tokenwise: warning — {warning}")
    if not decision.findings:
        lines.append("tokenwise: no rule had an opinion on this event.")
    return "\n".join(lines)
