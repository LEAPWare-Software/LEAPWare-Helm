...
plugin (`plugins/claude/tokenwise/hooks/hooks.json`) registers an enforcing
...
ships reporting-only: `skills/tokenwise-config` and `skills/tokenwise-report`, both
backed by `adapters/codex/hook_io.py` and the same `tokenwise_core` engine the
