"""Codex adapter: translate Codex's hook JSON <-> lwt_core's neutral shapes.
...
Claude adapter: see `plugins/codex/lwt/bin/lwt_hook.py` for the thin script
that reads stdin / writes stdout / sets the exit code.

`render_report` and the lwt-report skill are kept: a human-readable report
...
from lwt_core.config import Policy, load_policy_dict
from lwt_core.engine import Decision
from lwt_core.events import Event

#: Codex's env var naming the installed plugin's own root directory, used in
#: hooks.json command args (e.g. "${PLUGIN_ROOT}/bin/lwt_hook.py"). Per the
...
#: Claude's CLAUDE_PLUGIN_DATA in plugins/claude/lwt/bin/lwt_hook.py).
...
def render_report(decision: Decision) -> str:
    """Human-readable text for the lwt-report skill. Reporting only — never blocks."""
    lines = []
    if decision.permit:
        lines.append("lwt: would ALLOW under current policy.")
    else:
        lines.append(f"lwt: would DENY under current policy — {decision.deny_reason}")
    for warning in decision.warnings:
        lines.append(f"lwt: warning — {warning}")
    if not decision.findings:
        lines.append("lwt: no rule had an opinion on this event.")
    return "\n".join(lines)
