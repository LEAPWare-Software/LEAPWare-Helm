...
equivalent of `plugins/claude/lwt/hooks/hooks.json`. Both plugins run the
same `lwt_core` engine and the same rules; only the wire format differs.
...
- `hooks.json` — one `PreToolUse` hook, matcher `Agent`, running
  `bin/lwt_hook.py` (mirrors `plugins/claude/lwt/bin/lwt_hook.py`) via the
...
- `bin/lwt_hook.py` — reads the hook's stdin JSON, evaluates it through
  `lwt_core.engine.evaluate`, appends one ledger line under
