---
name: lwt-report
description: Run lwt's policy engine over a described action and report what it WOULD decide, without waiting for the real PreToolUse hook to fire (Codex side; see hooks/hooks.json for the enforcing hook itself).
---

# lwt-report (Codex)

The Codex plugin now also ships an enforcing `PreToolUse` hook
(`hooks/hooks.json`, running `bin/lwt_hook.py`), the same mechanism the
...
prompt), it runs `lwt_core.engine.evaluate` via `adapters/codex/hook_io.py`
...
2. Load the active policy the same way lwt-config does.
3. Run `adapters.codex.hook_io.parse_event`, then `lwt_core.engine.evaluate`,
