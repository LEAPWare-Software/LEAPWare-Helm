---
name: lwt-config
description: Read or edit lwt's token-policy configuration for this session or project (Codex side; the same policy file the enforcing PreToolUse hook in hooks/hooks.json reads).
---

# lwt-config (Codex)

Identical policy format and file to the Claude Code plugin: see
`core/policy/schema.json` and `core/policy/default.json` in the lwt
...
3. Remind the user explicitly: a `deny` mode here is enforced immediately
   ...
   Claude Code plugin — see lwt-report to preview a decision without
