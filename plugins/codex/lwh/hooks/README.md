...
equivalent of `plugins/claude/lwh/hooks/hooks.json`. Both plugins run the
same `lwh_core` engine and the same rules; only the wire format differs.
...
- `hooks.json` — one `PreToolUse` hook, matcher `Agent`, running
  `bin/lwh_hook.py` (mirrors `plugins/claude/lwh/bin/lwh_hook.py`) via the
...
- `bin/lwh_hook.py` — reads the hook's stdin JSON, evaluates it through
  `lwh_core.engine.evaluate`, appends one ledger line under
