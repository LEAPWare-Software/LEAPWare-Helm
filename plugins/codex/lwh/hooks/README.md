...
plugin (`plugins/claude/lwh/hooks/hooks.json`) registers an enforcing
...
ships reporting-only: `skills/lwh-config` and `skills/lwh-report`, both
backed by `adapters/codex/hook_io.py` and the same `lwh_core` engine the
