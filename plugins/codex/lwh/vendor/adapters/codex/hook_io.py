...
def render_report(decision: Decision) -> str:
    """Human-readable text for the lwh-report skill. Reporting only — never blocks."""
    lines = []
    if decision.permit:
        lines.append("lwh: would ALLOW under current policy.")
    else:
        lines.append(f"lwh: would DENY under current policy — {decision.deny_reason}")
    for warning in decision.warnings:
        lines.append(f"lwh: warning — {warning}")
    if not decision.findings:
        lines.append("lwh: no rule had an opinion on this event.")
    return "\n".join(lines)
