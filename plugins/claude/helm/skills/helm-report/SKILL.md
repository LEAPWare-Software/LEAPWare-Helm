---
name: helm-report
description: Summarize helm's decision ledger for this session (what was allowed, warned, or denied, and why). Use when the user asks "what has helm blocked", "show the spend policy log", or similar.
---

# helm-report

helm appends one JSON line per evaluated hook event to a ledger file (see
`helm_core/ledger.py` for the record shape: timestamp, hook_event,
tool_name, permit, deny_reason, warnings, findings).

## Locating the ledger

1. Check `HELM_LEDGER_PATH` in the environment; if set, that file is the
   ledger.
