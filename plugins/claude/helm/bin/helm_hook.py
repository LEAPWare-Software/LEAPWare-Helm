...
lives in helm_core or adapters/claude/hook_io.py, both under vendor/ next to
...
from helm_core.engine import evaluate  # noqa: E402
from helm_core.ledger import ledger_record  # noqa: E402


def _resolve_policy_path() -> Path:
    """User override policy, if present, else the bundled default.

    Reads `HELM_POLICY_PATH` if set (for tests and advanced setups, since a
    ...
    override = os.environ.get("HELM_POLICY_PATH")
    if override:
...
def _load_policy_dict(path: Path):
    ...
    than raising, per the fail-open contract in helm_core/config.py.
    """
    try:
...
def _ledger_path() -> Path:
    ...
    override = os.environ.get("HELM_LEDGER_PATH")
    if override:
        return Path(override)
