---
name: lwt-config
description: Read or edit lwt's token-policy configuration (which rules are off/warn/deny) for this session or project. Use when the user asks to change a lwt rule's mode, add a rule, or explain what the current policy will do.
---

# lwt-config

lwt's policy is one JSON file matching `core/policy/schema.json`: a
...
1. Check `LWT_POLICY_PATH` in the environment; if set, that file is active.
...
1. Locate or create the policy file (project convention: `.lwt/policy.json`
...
This is deliberate (see `core/lwt_core/config.py`), not a bug to route
