...
lives in lwt_core or adapters/claude/hook_io.py, both under vendor/ next to
this file (populated by scripts/lwt_build.py). This file only does I/O and
...
def _resolve_policy_path() -> Path:
    ...
    Reads `LWT_POLICY_PATH` if set (for tests and advanced setups, since a
...
def _load_policy_dict(path: Path):
    ...
    than raising, per the fail-open contract in lwt_core/config.py.
    """
    try:
