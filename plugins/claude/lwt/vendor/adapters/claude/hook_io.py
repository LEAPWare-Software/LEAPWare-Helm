"""Claude Code adapter: translate Claude's hook JSON <-> lwt_core's neutral shapes.
...
stdout, or call sys.exit — see `plugins/claude/lwt/bin/lwt_hook.py` for the
...
from lwt_core.config import Policy
...
#: hooks.json command args (e.g. "${CLAUDE_PLUGIN_ROOT}/bin/lwt_hook.py").
...
def load_policy(raw: Any) -> Policy:
    ...
    Kept here (rather than making bin/lwt_hook.py import lwt_core.config
    directly) so every Claude-facing translation lives in one file.
    """
    from lwt_core.config import load_policy_dict
