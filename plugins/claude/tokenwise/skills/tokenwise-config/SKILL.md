...
description: Read or edit tokenwise's token-policy configuration (which rules are off/warn/deny) for this session or project. Use when the user asks to change a tokenwise rule's mode, add a rule, or explain what the current policy will do.
---

# tokenwise-config

tokenwise's policy is one JSON file matching `core/policy/schema.json`: a
...
1. Locate or create the policy file (project convention: `.tokenwise/policy.json`
...
This is deliberate (see `core/tokenwise_core/config.py`), not a bug to route
