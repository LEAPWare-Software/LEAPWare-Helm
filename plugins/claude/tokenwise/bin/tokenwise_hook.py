...
lives in tokenwise_core or adapters/claude/hook_io.py, both under vendor/ next to
this file (populated by scripts/build.py). This file only does I/O and
...
from tokenwise_core.engine import evaluate  # noqa: E402
from tokenwise_core.ledger import ledger_record  # noqa: E402


def _resolve_policy_path() -> Path:
    """User override policy, if present, else the bundled default.

    Reads `TOKENWISE_POLICY_PATH` if set (for tests and advanced setups, since a
    ...
    override = os.environ.get("TOKENWISE_POLICY_PATH")
    if override:
...
def _load_policy_dict(path: Path):
    ...
    than raising, per the fail-open contract in tokenwise_core/config.py.
    """
    try:
...
def _ledger_path() -> Path:
    ...
    override = os.environ.get("TOKENWISE_LEDGER_PATH")
    if override:
        return Path(override)
