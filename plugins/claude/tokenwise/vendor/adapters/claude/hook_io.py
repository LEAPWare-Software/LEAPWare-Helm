"""Claude Code adapter: translate Claude's hook JSON <-> tokenwise_core's neutral shapes.
...
stdout, or call sys.exit — see `plugins/claude/tokenwise/bin/tokenwise_hook.py` for the
...
#: hooks.json command args (e.g. "${CLAUDE_PLUGIN_ROOT}/bin/tokenwise_hook.py").
...
def load_policy(raw: Any) -> Policy:
    ...
    Kept here (rather than making bin/tokenwise_hook.py import tokenwise_core.config
    directly) so every Claude-facing translation lives in one file.
    """
    from tokenwise_core.config import load_policy_dict
