"""Claude Code adapter: translate Claude's hook JSON <-> lwh_core's neutral shapes.
...
stdout, or call sys.exit — see `plugins/claude/lwh/bin/lwh_hook.py` for the
...
from lwh_core.config import Policy
...
#: hooks.json command args (e.g. "${CLAUDE_PLUGIN_ROOT}/bin/lwh_hook.py").
...
def load_policy(raw: Any) -> Policy:
    ...
    Kept here (rather than making bin/lwh_hook.py import lwh_core.config
    directly) so every Claude-facing translation lives in one file.
    """
    from lwh_core.config import load_policy_dict
