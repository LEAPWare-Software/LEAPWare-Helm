---
name: lwh-config
description: Read or edit lwh's token-policy configuration (which rules are off/warn/deny) for this session or project. Use when the user asks to change a lwh rule's mode, add a rule, or explain what the current policy will do.
---
...
1. Check `LWH_POLICY_PATH` in the environment; if set, that file is active.
...
1. Locate or create the policy file (project convention: `.lwh/policy.json`
...
This is deliberate (see `core/lwh_core/config.py`), not a bug to route
