"""Claude Code adapter: translate Claude's hook JSON <-> helm_core's neutral shapes.
...
stdout, or call sys.exit — see `plugins/claude/helm/bin/helm_hook.py` for the
...
#: hooks.json command args (e.g. "${CLAUDE_PLUGIN_ROOT}/bin/helm_hook.py").
...
def load_policy(raw: Any) -> Policy:
    ...
    Kept here (rather than making bin/helm_hook.py import helm_core.config
    directly) so every Claude-facing translation lives in one file.
    """
    from helm_core.config import load_policy_dict
