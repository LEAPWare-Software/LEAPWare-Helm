"""Unit tests for scripts/lwh_handoff.py. No network; git/gh calls are
...
SCRIPT_PATH = REPO_ROOT / "scripts" / "lwh_handoff.py"

_spec = importlib.util.spec_from_file_location("lwh_handoff", SCRIPT_PATH)
lwh_handoff = importlib.util.module_from_spec(_spec)
sys.modules["lwh_handoff"] = lwh_handoff
_spec.loader.exec_module(lwh_handoff)


def _valid_text(begin: str = lwh_handoff.BEGIN_MARKER, end: str = lwh_handoff.END_MARKER) -> str:
    return (
        "# HANDOFF\n\n"
        "## Start of session\n- [ ] read this\n\n"
        "## In flight\n1. do the thing\n\n"
...
def test_cmd_check_missing_file(tmp_path, monkeypatch, capsys):
    monkeypatch.setattr(lwh_handoff, "HANDOFF_PATH", tmp_path / "HANDOFF.md")
    assert lwh_handoff.cmd_check() == 1
...
def test_cmd_check_valid_file(tmp_path, monkeypatch, capsys):
    ...
    assert lwh_handoff.cmd_check() == 0
...
def test_cmd_write_regenerates_only_the_block(tmp_path, monkeypatch):
    ...
    prose_before = before.split(lwh_handoff.BEGIN_MARKER)[0]

    assert lwh_handoff.cmd_write() == 0

    after = path.read_text(encoding="utf-8")
    prose_after = after.split(lwh_handoff.BEGIN_MARKER)[0]
    assert prose_before == prose_after
    assert "cafef00d" in after
    assert after.count(lwh_handoff.BEGIN_MARKER) == 1
    assert after.count(lwh_handoff.END_MARKER) == 1


def test_cmd_write_requires_existing_markers(tmp_path, monkeypatch, capsys):
    path = tmp_path / "HANDOFF.md"
    path.write_text(_valid_text().replace(lwh_handoff.BEGIN_MARKER, ""), encoding="utf-8")
    monkeypatch.setattr(lwh_handoff, "HANDOFF_PATH", path)
    assert lwh_handoff.cmd_write() == 1
...
def test_real_handoff_md_passes_check():
    ...
    assert lwh_handoff._validate(text) == []
