...
def test_forbidden_substrings_default_only_when_file_absent(tmp_path, monkeypatch):
    monkeypatch.setattr(lwt_handoff, "REPO_ROOT", tmp_path)
    assert lwt_handoff._forbidden_substrings() == lwt_handoff.DEFAULT_FORBIDDEN_SUBSTRINGS


def test_forbidden_substring_from_local_file_fails(tmp_path, monkeypatch):
    (tmp_path / lwt_handoff.PRIVATE_NAMES_FILE).write_text("AdoptersSecretProject\n", encoding="utf-8")
    monkeypatch.setattr(lwt_handoff, "REPO_ROOT", tmp_path)
...
def test_cmd_write_records_cli_and_session(tmp_path, monkeypatch):
    ...
    path.write_text(_valid_text(), encoding="utf-8")
    monkeypatch.setattr(lwt_handoff, "HANDOFF_PATH", path)
    monkeypatch.setattr(lwt_handoff, "PROOF_DIR", tmp_path / "no-such-proof-dir")
    monkeypatch.setattr(lwt_handoff, "_run_git", lambda args: "cafef00d")
    monkeypatch.setattr(lwt_handoff, "_run_gh", lambda args: "")

    assert lwt_handoff.cmd_write(cli="claude", session="sess-123") == 0
...
def test_cmd_write_lists_deliverable_proof_state(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(lwt_handoff, "HANDOFF_PATH", path)
    monkeypatch.setattr(lwt_handoff, "PROOF_DIR", proof_dir)
    monkeypatch.setattr(lwt_handoff, "_run_git", lambda args: "cafef00d")
    monkeypatch.setattr(lwt_handoff, "_run_gh", lambda args: "")
