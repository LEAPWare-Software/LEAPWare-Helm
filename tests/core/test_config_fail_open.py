"""Tests for the documented fail-open contract in tokenwise_core/config.py."""

from tokenwise_core.config import Policy, RuleMode, load_policy_dict
...
def test_none_policy_is_all_off_and_degraded():
    ...
    assert policy.rule_config("budget_line").mode is RuleMode.OFF
...
def test_unknown_rule_mode_string_coerces_to_off():
    policy = load_policy_dict({"rules": {"budget_line": {"mode": "block-everything"}}})
    assert policy.rule_config("budget_line").mode is RuleMode.OFF
...
def test_valid_policy_round_trips_mode_and_options():
    policy = load_policy_dict(
        {"rules": {"budget_line": {"mode": "deny", "options": {"x": 1}}}}
    )
    assert policy.degraded is False
    cfg = policy.rule_config("budget_line")
