"""Tests for scripts/lwt_check_proof.py against tests/fixtures/proof/.
...
import lwt_check_proof  # noqa: E402
...
def test_valid_record_has_no_errors():
    errors = lwt_check_proof._validate_record(FIXTURES / "valid.json", _load("valid.json"))
    assert errors == []


def test_self_certified_record_is_rejected():
    errors = lwt_check_proof._validate_record(FIXTURES / "self_certified.json", _load("self_certified.json"))
    assert any("self-certified" in e for e in errors)


def test_exit_mismatch_record_is_rejected():
    errors = lwt_check_proof._validate_record(FIXTURES / "exit_mismatch.json", _load("exit_mismatch.json"))
    assert any("expect_exit" in e for e in errors)


def test_missing_field_record_is_rejected():
    errors = lwt_check_proof._validate_record(FIXTURES / "missing_field.json", _load("missing_field.json"))
...
def test_real_proof_directory_validates_clean():
    ...
    # (LWT-D0.json, LWT-D2.json, ...); the validator must find every one of
    # them clean, not a failure.
    errors, count = lwt_check_proof.validate_all()
    assert errors == []
    assert count > 0
