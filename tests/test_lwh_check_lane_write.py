"""Tests for scripts/lwh_check_lane_write.py's `evaluate` against fixtures.
...
from lwh_check_lane_write import evaluate  # noqa: E402
...
def test_claude_edit_out_of_lane_denied():
    ...
    assert "plugins/codex/lwh/bin/lwh_hook.py" in out["hookSpecificOutput"]["permissionDecisionReason"]
...
def test_codex_apply_patch_out_of_lane_denied():
    ...
    assert out["hookSpecificOutput"]["permissionDecision"] == "deny"
    assert "plugins/claude/lwh/bin/lwh_hook.py" in out["hookSpecificOutput"]["permissionDecisionReason"]
...
def test_non_edit_tool_is_ignored():
    out = evaluate("claude", {"hook_event_name": "PreToolUse", "tool_name": "Read", "tool_input": {"file_path": "plugins/codex/lwh/bin/lwh_hook.py"}})
    assert out["hookSpecificOutput"]["permissionDecision"] == "allow"
