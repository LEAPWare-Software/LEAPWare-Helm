...
5. Run `py -3.12 scripts/build.py` so both plugins' `vendor/` trees pick up
   the new rule, then `py -3.12 scripts/build.py --check` to confirm.

## Workflow

```
py -3.12 -m pytest -q
py -3.12 scripts/build.py --check
...
- Do not hand-edit a `vendor/` directory — it is generated by
  `scripts/build.py` and CI will catch drift.
