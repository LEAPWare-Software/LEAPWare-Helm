...
1. Write it under `core/lwh_core/rules/<rule_id>.py`. It must expose
   `rule_id: str` and `evaluate(event, config) -> Optional[Finding]`, and it
   must be pure — no file I/O, no network, no clock.
2. Register it in `core/lwh_core/rules/__init__.py`'s `RULES` list.
...
5. Run `python scripts/lwh_build.py` so both plugins' `vendor/` trees pick up
   the new rule, then `python scripts/lwh_build.py --check` to confirm.

## Workflow

```
python -m pytest -q
python scripts/lwh_build.py --check
...
- Do not hand-edit a `vendor/` directory — it is generated by
  `scripts/lwh_build.py` and CI will catch drift.
