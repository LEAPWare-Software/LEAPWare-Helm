#!/usr/bin/env python3
"""CI check `lwh-proof`: every proof/*.json record is well-formed.
...
Stdlib only. Exits 0 and prints "lwh-proof check passed" (or a skipped
...
def main() -> int:
    ...
    if errors:
        ...
        return 1
    if count == 0:
        print("lwh-proof check skipped (no proof/*.json records yet)")
    else:
        print(f"lwh-proof check passed ({count} record(s))")
    return 0
