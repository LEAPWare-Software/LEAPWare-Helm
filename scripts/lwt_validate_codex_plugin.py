#!/usr/bin/env python3
"""Validate plugins/codex/lwt against the shape this project's Codex plugin uses.

Checks (stdlib only):
  - .codex-plugin/plugin.json exists, is valid JSON, has name/version/
    description, and its "hooks" field points at "./hooks/hooks.json".
  - skills/lwt-config/SKILL.md and skills/lwt-report/SKILL.md exist.
  - hooks/hooks.json exists, is valid JSON, and every command (including
    ...
    absolute path — mirrors scripts/lwt_validate_claude_plugin.py's
  ...
  - bin/lwt_hook.py exists.
  - vendor/lwt_core and vendor/adapters/codex exist (scripts/lwt_build.py has
...
PLUGIN_DIR = REPO_ROOT / "plugins" / "codex" / "lwt"
...
def validate() -> list[str]:
    ...
    for skill in ("lwt-config", "lwt-report"):
    ...
    if not (PLUGIN_DIR / "bin" / "lwt_hook.py").is_file():
        errors.append("missing plugins/codex/lwt/bin/lwt_hook.py")

    vendor_core = PLUGIN_DIR / "vendor" / "lwt_core"
    vendor_adapter = PLUGIN_DIR / "vendor" / "adapters" / "codex"
    if not vendor_core.is_dir():
        errors.append(f"missing {vendor_core} — run scripts/lwt_build.py")
    if not vendor_adapter.is_dir():
        errors.append(f"missing {vendor_adapter} — run scripts/lwt_build.py")
    ...
    if isinstance(marketplace, dict):
        ...
        if "./plugins/codex/lwt" not in paths:
            errors.append(
