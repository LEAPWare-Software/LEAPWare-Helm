...
and `.codex/hooks.json` (see those files), separate from the lwh PRODUCT's
...
`scripts/lwh_lanes.py` uses for the commit-level `lwh-lanes` CI check. The
...
record (see module docstring); the `lwh-lanes` CI check still catches an
out-of-lane commit regardless of what this hook did or didn't catch.

Usage:
    python scripts/lwh_check_lane_write.py --agent claude
    python scripts/lwh_check_lane_write.py --agent codex
...
from lwh_lanes import REPO_ROOT, classify_path  # noqa: E402
        ...
        return {
            "hookSpecificOutput": {
                ...
                "permissionDecisionReason": (
                    f"lwh lane guard: {agent} may not write outside its lane or shared "
