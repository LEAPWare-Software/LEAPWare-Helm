#!/usr/bin/env python3
"""CI check `lwt-env-leak`: no local-environment or private-project leak.
...
Usage:
    python scripts/lwt_check_env_leak.py
    python scripts/lwt_check_env_leak.py --range <base-sha>..<head-sha>

Stdlib only. Exits 0 and prints "lwt-env-leak check passed" on success;
...
def main() -> int:
    ...
    if findings:
        for f in findings:
            print(f"FAIL: {f}")
        return 1
    print("lwt-env-leak check passed")
    return 0
