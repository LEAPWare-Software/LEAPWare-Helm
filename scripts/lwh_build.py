...
for a Codex plugin package). So `lwh_core` and the relevant adapter are
copied — not symlinked, copied, since a symlink does not survive a zip
release artifact — into `plugins/claude/lwh/vendor/` and
`plugins/codex/lwh/vendor/` respectively. This script is the ONLY place
that copy happens; nobody should hand-edit a vendor/ directory.

Usage:
    python scripts/lwh_build.py            # write/refresh both vendor/ trees
    python scripts/lwh_build.py --check    # exit 1 if a vendor/ tree is stale
...
TARGETS = [
    ("claude", REPO_ROOT / "plugins" / "claude" / "lwh" / "vendor"),
    ("codex", REPO_ROOT / "plugins" / "codex" / "lwh" / "vendor"),
]
...
def _build_one(host: str, vendor_dir: Path, tmp_root: Path) -> Path:
    ...
    shutil.copytree(CORE_DIR / "lwh_core", staging / "lwh_core", ignore=_IGNORE_PATTERNS)
...
def main() -> int:
    ...
    drift_found = False
    with tempfile.TemporaryDirectory(prefix="lwh-build-") as tmp:
        tmp_root = Path(tmp)
        for host, vendor_dir in TARGETS:
            ...
            if args.check:
                if not vendor_dir.exists() or not _trees_equal(staging, vendor_dir):
                    print(f"DRIFT: {vendor_dir} does not match source (run scripts/lwh_build.py)")
                    drift_found = True
                else:
