#!/usr/bin/env python3
"""CI check `lwh-prefix`: every user-facing entrypoint starts with `lwh`.

Checks (stdlib only):
  - Every `skills/<name>/SKILL.md` directory name under `plugins/**` starts
    with `lwh-`, and its front-matter `name:` field matches the directory.
  - Every `command`, `bin/`, or `scripts/` script a user or plugin manifest
    can invoke directly — `plugins/*/*/bin/*.py` and `scripts/*.py` — starts
    with `lwh`, except this script's own module (checked by name) and
    non-entrypoint helpers explicitly allow-listed below.
  - Both plugin manifests' `"name"` field is exactly `lwh`.

Exits 0 and prints "lwh-prefix check passed" on success; otherwise prints
...
def _check_skills(errors: list[str]) -> None:
    for skill_md in sorted(REPO_ROOT.glob("plugins/*/*/skills/*/SKILL.md")):
        ...
        if not dir_name.startswith("lwh-"):
            errors.append(f"skill directory does not start with 'lwh-': {skill_md.parent}")
            continue
...
def _check_scripts(errors: list[str]) -> None:
    for py in sorted(REPO_ROOT.glob("scripts/*.py")):
        if py.name in _NON_ENTRYPOINT_EXEMPT:
            continue
        if not py.name.startswith("lwh"):
            errors.append(f"scripts/ entrypoint does not start with 'lwh': {py}")

    for py in sorted(REPO_ROOT.glob("plugins/*/*/bin/*.py")):
        if py.name in _NON_ENTRYPOINT_EXEMPT:
            continue
        if not py.name.startswith("lwh"):
            errors.append(f"plugin bin/ entrypoint does not start with 'lwh': {py}")
...
def _check_manifests(errors: list[str]) -> None:
    for manifest_path in sorted(REPO_ROOT.glob("plugins/*/*/.claude-plugin/plugin.json")) + sorted(
        ...
        try:
            ...
            continue
        if data.get("name") != "lwh":
            errors.append(f"{manifest_path}: plugin id is '{data.get('name')}', want 'lwh'")
    ...
    for marketplace_path in (
        ...
        for entry in data.get("plugins", []):
            if isinstance(entry, dict) and entry.get("name") != "lwh":
                errors.append(
                    f"{marketplace_path}: plugin entry id is '{entry.get('name')}', want 'lwh'"
                )
