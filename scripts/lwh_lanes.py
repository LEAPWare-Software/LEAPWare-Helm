#!/usr/bin/env python3
"""Lane classification shared by the `lwh-lanes` CI check and the in-repo
session lane hooks (`scripts/lwh_check_lane_write.py`).
...
def commit_agent(sha: str) -> Optional[str]:
    """The `LWH-Agent:` trailer value for `sha`, or None if missing/invalid."""
...
def check_lanes(rev_range: str, pr_number: int) -> list[str]:
    ...
    for sha in commits_in_range(rev_range):
        ...
        if agent is None:
            errors.append(f"{sha[:12]}: missing or invalid 'LWH-Agent:' trailer")
            continue
        if agent == "human":
        ...
        for f in files:
            ...
            if cls == "shared":
                ...
                errors.append(
                    f"{sha[:12]} (LWH-Agent: {agent}): touches '{f}', outside the "
...
def main() -> int:
    ...
    if errors:
        ...
        return 1
    if args.pr_number == 0:
        print("lwh-lanes check skipped (not running under a PR)")
    elif args.pr_number <= BOOTSTRAP_LAST_EXEMPT_PR:
        print(f"lwh-lanes check skipped (bootstrap exception, PR #{args.pr_number})")
    else:
        print("lwh-lanes check passed")
    return 0
