...
Usage:
    python scripts/lwt_handoff.py --check                      # validate HANDOFF.md, exit 1 on failure
    python scripts/lwt_handoff.py --write [--cli NAME] [--session ID]
...
BEGIN_MARKER = "<!-- lwt-handoff:begin -->"
END_MARKER = "<!-- lwt-handoff:end -->"
...
# gitignored file (PRIVATE_NAMES_FILE, shared with scripts/lwt_check_env_leak.py)
...
def _proof_state_lines() -> list[str]:
    ...
    Reuses scripts/lwt_check_proof.py's own record validator so this never
    drifts from what the `lwt-proof` CI job itself enforces. An empty
    proof/ directory (the common case today -- see proof/README.md) is not
    an error; it just means the list is "(none yet)".
    """
    import lwt_check_proof  # local import: only --write needs this, --check must not
    ...
    for path in records:
        try:
            ...
            continue
        errors = lwt_check_proof._validate_record(path, data)
