#!/usr/bin/env python3
"""CI check `lwt-proof`: every proof/*.json record is well-formed.
...
Stdlib only. Exits 0 and prints "lwt-proof check passed" (or a skipped
...
def _validate_record(path: Path, data: object) -> list[str]:
    ...
    try:
        ...
        # or lwt_handoff.py's PROOF_DIR monkeypatched for a test) -- fall
...
def main() -> int:
    ...
    if errors:
        ...
        return 1
    if count == 0:
        print("lwt-proof check skipped (no proof/*.json records yet)")
    else:
        print(f"lwt-proof check passed ({count} record(s))")
    return 0
