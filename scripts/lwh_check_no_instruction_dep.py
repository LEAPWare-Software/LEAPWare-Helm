#!/usr/bin/env python3
"""CI check `lwh-no-instruction-dep`: runtime code never names CLAUDE.md or
AGENTS.md.
...
Usage:
    python scripts/lwh_check_no_instruction_dep.py

Stdlib only. Exits 0 and prints "lwh-no-instruction-dep check passed" on
...
def main() -> int:
    ...
    if violations:
        for v in violations:
            print(f"FAIL: {v}")
        return 1
    print("lwh-no-instruction-dep check passed")
    return 0
