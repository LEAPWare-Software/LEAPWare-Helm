...
for a Codex plugin package). So `lwt_core` and the relevant adapter are
copied — not symlinked, copied, since a symlink does not survive a zip
release artifact — into `plugins/claude/lwt/vendor/` and
`plugins/codex/lwt/vendor/` respectively. This script is the ONLY place
that copy happens; nobody should hand-edit a vendor/ directory.

Usage:
    python scripts/lwt_build.py            # write/refresh both vendor/ trees
    python scripts/lwt_build.py --check    # exit 1 if a vendor/ tree is stale
...
TARGETS = [
    ("claude", REPO_ROOT / "plugins" / "claude" / "lwt" / "vendor"),
    ("codex", REPO_ROOT / "plugins" / "codex" / "lwt" / "vendor"),
]
...
def _build_one(host: str, vendor_dir: Path, tmp_root: Path) -> Path:
    ...
    shutil.copytree(CORE_DIR / "lwt_core", staging / "lwt_core", ignore=_IGNORE_PATTERNS)
...
def main() -> int:
    ...
    drift_found = False
    with tempfile.TemporaryDirectory(prefix="lwt-build-") as tmp:
        tmp_root = Path(tmp)
        for host, vendor_dir in TARGETS:
            ...
            if args.check:
                if not vendor_dir.exists() or not _trees_equal(staging, vendor_dir):
                    print(f"DRIFT: {vendor_dir} does not match source (run scripts/lwt_build.py)")
                    drift_found = True
                else:
