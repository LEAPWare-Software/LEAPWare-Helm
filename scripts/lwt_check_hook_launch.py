#!/usr/bin/env python3
"""CI check backing the `lwh-portable` job: run each plugin's hook EXACTLY
as declared, for both Claude Code and Codex.

For each of `plugins/claude/lwt/hooks/hooks.json` and
`plugins/codex/lwt/hooks/hooks.json`, this takes the literal `command`
...
A deny-mode policy (`budget_line: deny`) is pointed at via `LWT_POLICY_PATH`
...
Usage:
    python scripts/lwt_check_hook_launch.py
...
def _check_one(plugin_dir: Path, root_var: str, fixture: Path, errors: list[str]) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="lwh-portable-") as tmp:
        ...
        env = dict(os.environ)
        env["LWT_POLICY_PATH"] = str(policy_path)
        env["LWT_LEDGER_PATH"] = str(Path(tmp) / "ledger.jsonl")
