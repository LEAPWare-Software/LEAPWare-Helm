...
and `.codex/hooks.json` (see those files), separate from the lwt PRODUCT's
...
`scripts/lwt_lanes.py` uses for the commit-level `lwt-lanes` CI check. The
...
record (see module docstring); the `lwt-lanes` CI check still catches an
out-of-lane commit regardless of what this hook did or didn't catch.

Usage:
    python scripts/lwt_check_lane_write.py --agent claude
    python scripts/lwt_check_lane_write.py --agent codex
...
from lwt_lanes import REPO_ROOT, classify_path  # noqa: E402
        ...
        return {
            "hookSpecificOutput": {
                ...
                "permissionDecisionReason": (
                    f"lwt lane guard: {agent} may not write outside its lane or shared "
