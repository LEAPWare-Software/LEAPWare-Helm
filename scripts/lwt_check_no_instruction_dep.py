#!/usr/bin/env python3
"""CI check `lwt-no-instruction-dep`: runtime code never names CLAUDE.md or
AGENTS.md.
...
Usage:
    python scripts/lwt_check_no_instruction_dep.py

Stdlib only. Exits 0 and prints "lwt-no-instruction-dep check passed" on
...
def main() -> int:
    ...
    if violations:
        for v in violations:
            print(f"FAIL: {v}")
        return 1
    print("lwt-no-instruction-dep check passed")
    return 0
