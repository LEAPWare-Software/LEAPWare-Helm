...
Usage:
    python scripts/build.py            # write/refresh both vendor/ trees
    python scripts/build.py --check    # exit 1 if a vendor/ tree is stale
        ...
        for host, vendor_dir in TARGETS:
            ...
            if args.check:
                if not vendor_dir.exists() or not _trees_equal(staging, vendor_dir):
                    print(f"DRIFT: {vendor_dir} does not match source (run scripts/build.py)")
                    drift_found = True
                else:
