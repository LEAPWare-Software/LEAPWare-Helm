#!/usr/bin/env python3
"""Lane classification shared by the `lwt-lanes` CI check and the in-repo
session lane hooks (`scripts/lwt_check_lane_write.py`).
...
into the `lwt-lanes` CI job) is a no-op for PR numbers 1-5 — the PR that
...
TRAILER_RE = re.compile(r"^LWT-Agent:\s*(claude|codex|human)\s*$", re.MULTILINE)
...
def commit_agent(sha: str) -> Optional[str]:
    """The `LWT-Agent:` trailer value for `sha`, or None if missing/invalid."""
...
def check_lanes(rev_range: str, pr_number: int) -> list[str]:
    ...
    for sha in commits_in_range(rev_range):
        ...
        if agent is None:
            errors.append(f"{sha[:12]}: missing or invalid 'LWT-Agent:' trailer")
            continue
        if agent == "human":
        ...
        for f in files:
            ...
            if cls == "shared":
                ...
                errors.append(
                    f"{sha[:12]} (LWT-Agent: {agent}): touches '{f}', outside the "
...
def main() -> int:
    ...
    if errors:
        ...
        return 1
    if args.pr_number == 0:
        print("lwt-lanes check skipped (not running under a PR)")
    elif args.pr_number <= BOOTSTRAP_LAST_EXEMPT_PR:
        print(f"lwt-lanes check skipped (bootstrap exception, PR #{args.pr_number})")
    else:
        print("lwt-lanes check passed")
    return 0
