#!/usr/bin/env python3
"""Validate plugins/claude/lwh against the shape Claude Code plugins require.
...
Checks (stdlib only):
  ...
  - vendor/lwh_core and vendor/adapters/claude exist (i.e. scripts/lwh_build.py
...
def validate() -> list[str]:
    ...
    if not (PLUGIN_DIR / "bin" / "lwh_hook.py").is_file():
        errors.append("missing plugins/claude/lwh/bin/lwh_hook.py")
    ...
    if isinstance(marketplace, dict):
        ...
        if "./plugins/claude/lwh" not in sources:
            errors.append(
