#!/usr/bin/env python3
"""Validate plugins/claude/lwt against the shape Claude Code plugins require.
...
Checks (stdlib only):
  ...
  - vendor/lwt_core and vendor/adapters/claude exist (i.e. scripts/lwt_build.py
...
PLUGIN_DIR = REPO_ROOT / "plugins" / "claude" / "lwt"
...
def validate() -> list[str]:
    ...
    if not (PLUGIN_DIR / "bin" / "lwt_hook.py").is_file():
        errors.append("missing plugins/claude/lwt/bin/lwt_hook.py")
    ...
    if isinstance(marketplace, dict):
        ...
        if "./plugins/claude/lwt" not in sources:
            errors.append(
